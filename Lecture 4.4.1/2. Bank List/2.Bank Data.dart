...
List<BankModel> BankCustomer = [];

void main() {
  for (int i = 0; i < bankData.length; i++) {
    BankModel b1 = BankModel.fromBank(Bank: bankData[i]);

    BankCustomer.add(b1);
  }

  for (int i = 0; i < BankCustomer.length; i++) {
    print('<---Custome no.: ${i + 1}--->\n');

    print('"isActive" : ${BankCustomer[i].x}\n' +
        '"balance" : ${BankCustomer[i].balance}\n' +
        '"age" : ${BankCustomer[i].age}\n' +
        '"eyeColor" : ${BankCustomer[i].eyeColor}\n' +
        '"name" : ${BankCustomer[i].name}\n' +
        '"gender" : ${BankCustomer[i].gender}\n' +
        '"company" : ${BankCustomer[i].company}\n' +
        '"email" : ${BankCustomer[i].email}\n' +
        '"phone" : ${BankCustomer[i].phone}\n' +
        '"address" : ${BankCustomer[i].address}\n\n');
  }
}
